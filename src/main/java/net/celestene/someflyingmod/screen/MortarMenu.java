...
public class MortarMenu extends AbstractContainerMenu {
    ...
    public MortarMenu(int pContainerID, Inventory inv, BlockEntity be, ContainerData data) {
        super(ModMenuTypes.MORTAR_MENU.get(), pContainerID);
        checkContainerSize(inv, 2);
        blockEntity = ((MortarBlockEntity) be);
        ...
        // .ifPresent() (below) protects from potential brief errors if item handler is not loaded yet when the GUI first opens (before synchronization)
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(iItemHandler -> {
            this.addSlot(new SlotItemHandler(iItemHandler, 0, 109, 19));
            this.addSlot(new SlotItemHandler(iItemHandler, 1, 152, 19));
        });
    ...
    public boolean isCrafting() {
        return data.get(0) > 0; // (is progress > 0) (since you defined case 0 as returning progress
    }

    public int getScaledProgress() {
        int progress = this.data.get(0);
        int maxProgress = this.data.get(1);
        int progressArrowSize = 15; // length of arrow
    ...
    // This method is responsible for shift clicking items in and out of your block entity
    @Override
